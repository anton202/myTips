...
export class WaitersBookComponent implements OnInit {
  ...
  waitrShift: FormGroup;

  constructor(
    private waitrsBookService: WaitrsBookService,
  ...
  private getWaitrsNames(): void {
    this.waitrsBookService
      .getWorkersNames()
      .subscribe(
        workersNames => (this.workersNames = workersNames),
        error =>
          this.handelError(
            "משהו השתבש כשניסינו לדלות את השמות של המלצרים מהשרת, נסה שוב."
          )
      );
  }

  private initializeForms(): void {
    this.totalAmount = new FormGroup({
      totalTips: new FormControl(null, Validators.required)
    });

    this.waitrShift = new FormGroup({
      waitrName: new FormControl(null, Validators.required),
  ...
  public addWaitr(): void {
    const waitrName = this.waitrShift.value.waitrName;
    const shiftStartTime = this.waitrShift.value.shiftStartTime;
    const shiftEndTime = this.waitrShift.value.shiftEndTime;

    if (this.waitrShift.valid) {
      this.waitrsStack.push(new Waitr(waitrName, shiftStartTime, shiftEndTime));
      this.isShiftTimeNotSet = false;
      this.isWaitrAdded = true;
      this.waitrShift.get('waitrName').reset();
    }

    if (
      !this.waitrShift.get("shiftStartTime").valid ||
      !this.waitrShift.get("shiftEndTime").valid
    ) {
  ...
  public calculateTips(): void {
    const totalTip = this.totalAmount.value.totalTips;
    ...
    const totalTime = this.waitrsBookService.calculateTotalTime(
      waitrsStackClone
    );
    ...
    this.moneyToEmployer = Math.floor(
      this.waitrsBookService.moneyToEmployer(totalTime, 6)
    );

    //set all waitrs per hour tip.
    this.waitrsBookService.setWaitrsPerHourTip(
      waitrsStackClone,
      totalTime,
      totalTipMutated
    );
    this.tipPerHour = this.waitrsBookService.tipPerHour(
      totalTipMutated,
      totalTime
    );
  ...
  private saveCalculatedTips(waitrsStack): void {
    this.waitrsBookService
      .sendDataToServer(waitrsStack)
      .subscribe(
        () => {},
        error =>
          this.handelError(
            "משהו השתבש כשניסינו לשמור את הטיפים על השרת, נסה שוב."
          )
      );
  }
