...
const style = wb.createStyle({
    ...
    numberFormat: 'U+20AA#,##0.00; (U+20AA#,##0.00); -',
...
const insertData = function(tips){
  for(let i = 0,c = 2,n=1; i < tips.length; i++){
    ws.cell(c,n=1).number(tips[i].moneyToGoverment)
    ws.cell(c,n+=1).number(tips[i].totalTip).style(style)
    ws.cell(c,n+=1).number(tips[i].perHour).style(style)
